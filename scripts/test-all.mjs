/**
 * SecurePolicy - Integration Test Suite (ES Module)
...
import { parseDocumentToMarkdown } from '../src/lib/document-parser.js';
import { splitMarkdownIntoChunks, buildSearchIndex, searchIndex } from '../src/lib/client-search.js';
import { getLocalDb } from '../src/lib/db/drivers/sqlite-local.js';
...
async function runTests() {
  console.log('🧪 Starting SecurePolicy ES Module Test Suite...\n');
  let passCount = 0;
  let failCount = 0;
  ...
  // --- Test 2: Document Parser (TXT & PDF) ---
  try {
    const textBuffer = Buffer.from('# Leave Policy\n\nEmployees get 20 days off.');
    const parsedText = await parseDocumentToMarkdown(textBuffer, 'text/plain', 'leaves.txt');
    assert(parsedText.startsWith('# Leave Policy'), 'Plain text parsed to markdown correctly');

    // Minimal valid 1-page PDF rendering "Hello World"
    ...
    const parsedPdf = await parseDocumentToMarkdown(minimalPdfBuffer, 'application/pdf', 'test.pdf');
    assert(parsedPdf.includes('Hello World'), 'PDF parsed to markdown correctly without DOMMatrix errors');
  } catch (e) {
...
Regular full-time staff accumulate 1.5 days of PTO per month.
    ...
    const results = searchIndex(indexJson, 'sick leaves', 2);
